...
require "slack_bot/observers/slash_event"
...
class SlackBot
  ...
  def observe_slash_event
    slash_event_observer.on do |response|
      SlackBot::Responder.new(
  ...
  %w(realtime slash_event).each do |name|
